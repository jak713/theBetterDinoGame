...
def create_player():
    ...
    player_id = db.insert_player(username)
...
def update_game_run(game_run_id):
    ...
    update_run = db.update_game_run(game_run_id, data["score"])

    if update_run:
        return jsonify({
            "message": "Game run updated"
        }), 200

    return jsonify({
        "error": "Game run not updated"
    }), 404
...
def delete_game_run(game_run_id):
    delete_row = db.delete_game_run(game_run_id)

    if delete_row:
        return jsonify({
            "message": "Game run deleted"
        }), 200
